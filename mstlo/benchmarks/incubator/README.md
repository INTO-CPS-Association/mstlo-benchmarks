...
4. **Start RabbitMQ.** From `6-PuttingItAllTogether`, run `python start_influxdb_rabbitmq.py`; `docker ps` should show `rabbitmq-server`. The broker address the services use is `rabbitmq.ip` in `incubator_dt/software/startup.conf`.
